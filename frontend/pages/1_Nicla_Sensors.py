...
with st.container():
    ...
    with st.container():
        ...
        with col5:
            fig5 = go.Figure()
            fig5.add_trace(go.Scatter(x=dates, y=co2_levels, mode='lines+markers', name='CO2 Level',
                                      line=dict(color='#2ca02c')))
            fig5.update_layout(title='CO2 Level', xaxis_title='Date', yaxis_title='CO2 (PPM)', template="plotly_white")
            st.plotly_chart(fig5, use_container_width=True)
        
        # Create a line chart eith gradient color
        with col6:
            ...
            fig6.update_layout(title='Air Quality', xaxis_title='Date', yaxis_title='AQI', template="plotly_white")
